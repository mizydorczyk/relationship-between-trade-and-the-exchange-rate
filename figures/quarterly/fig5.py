...
def plot(trade: pd.DataFrame, exchange_rates: pd.DataFrame, filename: str, category: str, label: str, detect_outliers_flag: bool = True):
    ...
    quarterly_avg = exchange_rates.groupby('Quarter')[['CNY', 'KRW']].mean().reset_index()
    quarterly_avg['Date'] = quarterly_avg['Quarter'].dt.to_timestamp()
    quarterly_avg['CNY_per_KRW'] = quarterly_avg['KRW'] / quarterly_avg['CNY']
    quarterly_avg['CNY_per_KRW_pct_change'] = quarterly_avg['CNY_per_KRW'].pct_change() * 100

    merged = pd.merge(trade_quarterly, quarterly_avg[['Date', 'CNY_per_KRW_pct_change']], on='Date', how='left')
    merged = merged.dropna()
    
    if detect_outliers_flag:
        trade_outliers = detect_outliers(merged[f'{category}_pct_change'], threshold=3.0)
        exchange_outliers = detect_outliers(merged['CNY_per_KRW_pct_change'], threshold=3.0)
    ...
    if len(normal_points) > 1:
        correlation = normal_points['CNY_per_KRW_pct_change'].corr(normal_points[f'{category}_pct_change'])
    ...
    ax.set_xlabel('Zmiana kursu CNY/KRW')
    ax.set_ylabel(f'Zmiana {label}')
    ax.set_xlim(normal_points['CNY_per_KRW_pct_change'].min(), normal_points['CNY_per_KRW_pct_change'].max())
...
def main():
    ...
    plot(trade, exchange_rates, 'fig5a.png', 'Exports', 'eksport Republiki Korei do Chin', detect_outliers_mode)
    plot(trade, exchange_rates, 'fig5b.png', 'Imports', 'import Republiki Korei z Chin', detect_outliers_mode)
    plot(trade, exchange_rates, 'fig5c.png', 'Balance', 'eksport netto Republiki Korei do Chin', detect_outliers_mode)
