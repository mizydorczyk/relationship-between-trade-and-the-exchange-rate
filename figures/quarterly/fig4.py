...
def plot(trade: pd.DataFrame, exchange_rates: pd.DataFrame, filename: str, category: str, label: str, detect_outliers_flag: bool = True):
    ...
    trade_quarterly = trade_quarterly.groupby('Quarter')[['Exports', 'Imports', 'Balance']].sum().reset_index()
    trade_quarterly['Date'] = trade_quarterly['Quarter'].dt.to_timestamp()

    exchange_rates['Quarter'] = exchange_rates['Date'].dt.to_period('Q')
    quarterly_avg = exchange_rates.groupby('Quarter')[['JPY', 'USD', 'CNY']].mean().reset_index()
    quarterly_avg['Date'] = quarterly_avg['Quarter'].dt.to_timestamp()

    quarterly_avg['CNY_per_JPY'] = quarterly_avg['JPY'] / quarterly_avg['CNY']
    quarterly_avg['CNY_per_JPY_pct_change'] = quarterly_avg['CNY_per_JPY'].pct_change() * 100

    merged = pd.merge(trade_quarterly, quarterly_avg[['Date', 'CNY_per_JPY_pct_change', 'JPY']], on='Date', how='left')
    merged[category] = merged[category] / quarterly_avg['JPY']
    merged[f'{category}_pct_change'] = merged[category].pct_change() * 100
    merged = merged.dropna()
    
    if detect_outliers_flag:
        trade_outliers = detect_outliers(merged[f'{category}_pct_change'], threshold=3.0)
        exchange_outliers = detect_outliers(merged['CNY_per_JPY_pct_change'], threshold=3.0)
    ...
    if len(normal_points) > 1:
        correlation = normal_points['CNY_per_JPY_pct_change'].corr(normal_points[f'{category}_pct_change'])
    ...
    ax.set_xlabel('Zmiana kursu CNY/JPY')
    ax.set_ylabel(f'Zmiana {label}')
    ax.set_xlim(normal_points['CNY_per_JPY_pct_change'].min(), normal_points['CNY_per_JPY_pct_change'].max())
...
def main():
    ...
    plot(trade, exchange_rates, 'fig4a.png', 'Exports', 'eksport Japonii do Chin', detect_outliers_mode)
    plot(trade, exchange_rates, 'fig4b.png', 'Imports', 'import Japonii z Chin', detect_outliers_mode)
    plot(trade, exchange_rates, 'fig4c.png', 'Balance', 'eksport netto Japonii do Chin', detect_outliers_mode)
